...
export const useUjianstudiPagesStore = defineStore(
  "ujianstudiPagesStore",
  () => {
    // State
    ...
    // State
    const siswa_profile = ref(
      localStorage.getItem("siswa_profile")
        ? JSON.parse(localStorage.getItem("siswa_profile"))
        : null
    );
    // Getter Setter
    ...
    const siswa_ujianstudi = ref(
      localStorage.getItem("siswa_ujianstudi")
        ? JSON.parse(localStorage.getItem("siswa_ujianstudi"))
        : null
    ); //![] semua mapel
    // Getter Setter
    ...
    // Getter Setter
    const get_siswa_ujianstudi_aktif = computed(
      () => siswa_ujianstudi_aktif.value
    ); //!{} satu mapel yang aktif
    const set_siswa_ujianstudi_aktif = (item) => {
      siswa_ujianstudi_aktif.value = item;
    };

    const siswa_ujianstudi_soal_aktif = ref(null);
    const get_siswa_ujianstudi_soal_aktif = computed(
      () => siswa_ujianstudi_soal_aktif.value
    );
    const set_siswa_ujianstudi_soal_aktif = (
      item,
      index = 0,
      is_updated = false
    ) => {
